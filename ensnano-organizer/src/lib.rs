...
/// Internal messages
#[derive(Clone, Debug)]
pub enum Message<E: OrganizerElement> {
    Expand {
...
impl<E: OrganizerElement> OrganizerMessage<E> {
    fn expand(id: NodeId<E::AutoGroup>, expanded: bool) -> Self {
        Self::InternalMessage(Message::Expand { id, expanded })
    }

    fn node_selected(id: NodeId<E::AutoGroup>) -> Self {
        Self::InternalMessage(Message::NodeSelected { id })
    }

    fn node_hovered(id: NodeId<E::AutoGroup>, hovered_in: bool) -> Self {
        Self::InternalMessage(Message::NodeHovered { id, hovered_in })
    }

    fn key_hovered(key: E::Key, hovered_in: bool) -> Self {
        Self::InternalMessage(Message::KeyHovered { key, hovered_in })
    }

    fn edit(id: NodeId<E::AutoGroup>) -> Self {
        Self::InternalMessage(Message::Edit { id })
    }

    fn delete(id: NodeId<E::AutoGroup>) -> Self {
        Self::InternalMessage(Message::Delete { id })
    }

    fn name_input(name: String) -> Self {
        Self::InternalMessage(Message::NameInput { name })
    }

    fn stop_edit() -> Self {
        Self::InternalMessage(Message::StopEdit)
    }

    fn element_selected(key: E::Key) -> Self {
        Self::InternalMessage(Message::ElementSelected { key })
    }

    fn add_selection_to_group(id: NodeId<E::AutoGroup>) -> Self {
        Self::InternalMessage(Message::AddSelectionToGroup { id })
    }

    fn new_group() -> Self {
        Self::InternalMessage(Message::NewGroup)
    }

    fn dragging(key: DragIdentifier<E::Key, E::AutoGroup>) -> Self {
        Self::InternalMessage(Message::Dragging(key))
    }

    fn drag_dropped(key: DragIdentifier<E::Key, E::AutoGroup>) -> Self {
        Self::InternalMessage(Message::DragDropped(key))
    }

    fn attribute_selected(attribute: E::Attribute, id: NodeId<E::AutoGroup>) -> Self {
        Self::InternalMessage(Message::AttributeSelected { attribute, id })
    }
}
...
impl<E: OrganizerElement> Organizer<E> {
    ...
    pub fn message(
        ...
        match &message {
            ...
            Message::NameInput { name } => self.edit_name(name.clone()),
            Message::StopEdit => {
                self.stop_editing();
                return Some(OrganizerMessage::NewTree(self.tree()));
            }
            Message::ElementSelected { key } => {
            ...
            Message::DragDropped(k) => self.drag_drop(k),
            Message::NodeHovered { id, hovered_in } => return self.hover(id, *hovered_in),
            Message::KeyHovered { key, hovered_in } => {
                return self.key_hover(key.clone(), *hovered_in);
            }
            Message::AttributeSelected { attribute, id } => {
