use crate::app_state::design_interactor::DesignInteractor;
use ahash::HashMap;
use ensnano_design::{
    curves::time_nucl_map::AbscissaConverter,
    domains::{Domain, helix_interval::HelixInterval},
    helices::{Helices, NuclCollection},
    nucl::Nucl,
    strands::Strand,
};
use ensnano_utils::torsion::Torsion;
use std::{collections::BTreeMap, sync::Arc};
use ultraviolet::{Isometry2, Vec2};

impl DesignInteractor {
    pub fn get_isometry(&self, h_id: usize, segment_idx: usize) -> Option<Isometry2> {
        if segment_idx == 0 {
            self.presenter
                .current_design
                .helices
                .get(&h_id)
                .and_then(|h| h.isometry2d)
        } else {
            self.presenter
                .current_design
                .helices
                .get(&h_id)
                .and_then(|h| h.additional_isometries.get(segment_idx - 1))
                .and_then(|i| i.additional_isometry)
        }
    }

    pub fn get_helix_segment_symmetry(&self, h_id: usize, segment_idx: usize) -> Option<Vec2> {
        if segment_idx == 0 {
            self.presenter
                .current_design
                .helices
                .get(&h_id)
                .map(|h| h.symmetry)
        } else {
            self.presenter
                .current_design
                .helices
                .get(&h_id)
                .and_then(|h| h.additional_isometries.get(segment_idx - 1))
                .and_then(|i| i.additional_symmetry)
        }
    }

    pub fn get_strand_points(&self, s_id: usize) -> Option<Vec<Nucl>> {
        let strand = self.presenter.current_design.strands.get(&s_id)?;
        let helices = &self.presenter.current_design.helices;
        let mut ret = Vec::new();
        for domain in &strand.domains {
            if let Domain::HelixDomain(domain) = domain {
                ret.extend(split_domain_into_helices_segment(domain, helices));
            }
        }
        if strand.is_cyclic {
            ret.push(ret[0]);
        }
        Some(ret)
    }

    pub fn get_all_strand_ids(&self) -> Vec<usize> {
        self.presenter
            .current_design
            .strands
            .keys()
            .copied()
            .collect()
    }

    pub fn get_strand_color(&self, s_id: usize) -> Option<u32> {
        self.presenter
            .current_design
            .strands
            .get(&s_id)
            .map(|s| s.color)
    }

    pub fn get_torsions(&self) -> HashMap<(Nucl, Nucl), Torsion> {
        HashMap::default()
    }

    pub fn get_basis_map(&self) -> Arc<HashMap<Nucl, char>> {
        self.presenter.content.letter_map.clone()
    }

    pub fn get_group_map(&self) -> Arc<BTreeMap<usize, bool>> {
        self.presenter.current_design.groups.clone()
    }

    pub fn get_insertions(&self, s_id: usize) -> Option<Vec<Nucl>> {
        self.presenter
            .current_design
            .strands
            .get(&s_id)
            .map(Strand::get_insertions)
    }

    pub fn get_copy_points(&self) -> Vec<Vec<Nucl>> {
        self.controller.get_copy_points()
    }

    pub fn get_visibility_helix(&self, h_id: usize) -> Option<bool> {
        self.presenter
            .current_design
            .helices
            .get(&h_id)
            .map(|h| h.visible)
    }

    pub fn get_xovers_list_with_id(&self) -> Vec<(usize, (Nucl, Nucl))> {
        self.presenter.junctions_ids.get_all_elements()
    }

    pub fn get_id_of_strand_containing_elt(&self, e_id: u32) -> Option<usize> {
        self.presenter.content.strand_map.get(&e_id).copied()
    }

    pub fn get_id_of_of_helix_containing_elt(&self, e_id: u32) -> Option<usize> {
        self.presenter.content.helix_map.get(&e_id).copied()
    }

    pub fn has_helix(&self, h_id: usize) -> bool {
        self.presenter.current_design.helices.contains_key(&h_id)
    }

    pub fn get_helices_map(&self) -> &Helices {
        &self.presenter.current_design.helices
    }

    pub fn get_strand_ends(&self) -> Vec<Nucl> {
        self.presenter
            .current_design
            .strands
            .values()
            .filter_map(|s| Some([s.get_5prime()?, s.get_3prime()?]))
            .flatten()
            .collect()
    }

    pub fn get_nucl_collection(&self) -> Arc<NuclCollection> {
        self.presenter.content.nucl_collection.clone()
    }

    pub fn get_abscissa_converter(&self, h_id: usize) -> AbscissaConverter {
        self.presenter
            .current_design
            .try_get_up_to_date()
            .map(|data| data.grid_data.get_abscissa_converter(h_id))
            .unwrap_or_default()
    }
}

fn split_domain_into_helices_segment(domain: &HelixInterval, helices: &Helices) -> Vec<Nucl> {
    let helix = helices.get(&domain.helix);
    let empty = vec![];
    let additional_segments = helix.map_or(&empty, |h| &h.additional_isometries);
    let mut ret = Vec::new();

    let mut iter = additional_segments
        .iter()
        .flat_map(|s| [s.left - 1, s.left])
        .skip_while(|pos| *pos <= domain.start);

    ret.push(Nucl {
        helix: domain.helix,
        forward: domain.forward,
        position: domain.start,
    });
    while let Some(position) = iter.next().filter(|pos| *pos < domain.end - 1) {
        ret.push(Nucl {
            helix: domain.helix,
            forward: domain.forward,
            position,
        });
    }
    ret.push(Nucl {
        helix: domain.helix,
        forward: domain.forward,
        position: domain.end - 1,
    });
    if !domain.forward {
        ret.reverse();
    }
    ret
}

// TODO
// #[cfg(test)]
// mod tests {

//     #[test]
//     fn correct_suggestions() {}

//     #[test]
//     fn get_torsions_implemented() {}

//     #[test]
//     fn get_correct_visibility_helix() {}
// }
