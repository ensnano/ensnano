            ...
            if pair.is_a_pair() {
                if let Some(ref mut range) = current_range {
                    if range.end == *position {
                        ...
                        self.double_ranges.push(range.clone());
...
mod tests {
    ...
    #[test]
    fn helix_ranges() {
        let mut helix = IntermediaryHelix {
            parameters: HelixParameters::GEARY_2014_DNA_P_STICK,
            helices: HashMap::default(),
            double_ranges: Vec::default(),
        };
