...
void main() {
    ...
    mat2 symmetry_mat = mat2(u_symmetry.x, 0.0, 0.0, u_symmetry.y);
    ...
    vec2 local_pos = instances[gl_InstanceIndex].center + ( rotation(-u_tilt) * symmetry_mat * a_position * size);

    vec2 world_pos = local_pos - u_scroll_offset; 
    vec2 zoom_factor = u_zoom / (vec2(0.5, 0.5) * u_resolution);
    vec2 transformed_pos = rotation(u_tilt) * world_pos * zoom_factor * u_symmetry * vec2(1., -1.);
