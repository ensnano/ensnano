use ensnano_utils::{bindgroup_manager::DynamicBindGroup, consts::SAMPLE_COUNT, texture::Texture};
use std::rc::Rc;
...
impl CircleDrawer {
    pub(crate) fn new(
        ...
        let pipeline = ret.create_pipeline(device, globals_layout, circle_kind);
    ...
    fn create_pipeline(
        ...
        let vertex_module = device.create_shader_module(include_spirv!("circle.vert.spv"));

        let fragment_module = match circle_kind {
            CircleKind::FullCircle => {
                device.create_shader_module(include_spirv!("circle.frag.spv"))
            }
            CircleKind::RotationWidget => {
                device.create_shader_module(include_spirv!("rotation_widget.frag.spv"))
            }
        };
